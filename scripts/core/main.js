...
class Game {
  constructor() {
    ...
    // Preparar pantalla inicial
    this.setupStartScreen();
    this.gameLoop();

    // Agregar método para configurar la pantalla inicial
  }
  setupStartScreen() {
    setTimeout(() => {
      this.startScreenReady = true;
    }, 1000); // Delay para asegurar que todo esté cargado
  };

  // Agregar nuevo método después del constructor
  async handleStartScreenInput(e) {
    if (!this.startScreenReady) return;
  ...
  // Agregar nuevo método para la transición
  async transitionToGame() {
    ...
    // Ocultar pantalla inicial y mostrar canvas
  ...
  // Agregar método para ocultar pantalla inicial
  hideStartScreen() {
  ...
  // Agregar método para mostrar canvas del juego
  showGameCanvas() {
  ...
  // Agregar método para inicializar el juego
  async initializeGame() {
    // Cargar el mapa inicial
    await this.loadMap('main');
    this.startScreenReady = true;
  }
  ...
  // Modificar el método update() - reemplazar completamente
  update() {
    // No hacer nada si estamos en la pantalla inicial
    if (this.showingStartScreen) {
      return;
    }
  ...
  draw() {
    // No dibujar nada si estamos en la pantalla inicial
    if (this.showingStartScreen) {
      return;
    }
